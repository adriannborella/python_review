...
class DynamicArray:
    ...
    def insert(self, index, value):
        ...
        # TODO: Consultar con señor copilot
        # Why >= instead of just >:
        # self.size > self.capacity should never happen in a correct implementation
        # self.size == self.capacity is the edge case where we're at full capacity
        # Using >= is defensive programming - it handles both the normal case and any potential bugs
        # So the resize happens preemptively when the array is full, ensuring we have enough space for the shifting operation and the new element.
        if self.size >= self.capacity:
            self._resize()
